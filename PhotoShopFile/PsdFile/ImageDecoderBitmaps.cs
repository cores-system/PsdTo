...
//        {
//            Bitmap bitmap = new Bitmap(psdFile.ColumnCount, psdFile.RowCount, PixelFormat.Format32bppArgb);
...
//        }
      ...
      for (int y = 0; y < layer.Rect.Height; y++)
      {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < layer.Rect.Height; y++)
                {
                    ...
                    for (int x = 0; x < layer.Rect.Width; x++)
                    {
                        ...
                        {
                            int maskAlpha = GetColor(layer.Masks, x, y);
        ...
        private static Color GetColor(Layer layer, int pos)
        {
            ...
            {
                case PsdColorMode.RGB:
                    if (3 > layer.SortedChannels.Count || layer.SortedChannels[0].ImageData == null)
                        return c;
        ...
        private static int GetColor(MaskInfo mask, int x, int y)
        {
            int c = 255;

            if (mask.LayerMask.PositionVsLayer)
            {
                x -= mask.LayerMask.Rect.X;
                y -= mask.LayerMask.Rect.Y;
            }
            else
            {
                x = (x + mask.LayerMask.Layer.Rect.X) - mask.LayerMask.Rect.X;
                y = (y + mask.LayerMask.Layer.Rect.Y) - mask.LayerMask.Rect.Y;
            }


            if (y >= 0 && y < mask.LayerMask.Rect.Height &&
                 x >= 0 && x < mask.LayerMask.Rect.Width)
            {
                int pos = y * mask.LayerMask.Rect.Width + x;
                if (mask.LayerMask.ImageData != null && pos < mask.LayerMask.ImageData.Length)
                    c = mask.LayerMask.ImageData[pos];
                else
                    c = 255;
            }
        ...
        public static Bitmap DecodeImage(MaskInfo mask)
        {
            Layer layer = mask.LayerMask.Layer;

            if (mask.LayerMask.Rect.Width == 0 || mask.LayerMask.Rect.Height == 0)
            {
                return null;
            }

            Bitmap bitmap = new Bitmap(mask.LayerMask.Rect.Width, mask.LayerMask.Rect.Height, PixelFormat.Format32bppArgb);
            ...
            unsafe
            {
                ...
                for (int y = 0; y < mask.LayerMask.Rect.Height; y++)
                {
                    int rowIndex = y * mask.LayerMask.Rect.Width;
                    PixelData* pCurrPixel = (PixelData*)pCurrRowPixel;
                    for (int x = 0; x < mask.LayerMask.Rect.Width; x++)
                    {
                        int pos = rowIndex + x;

                        Color pixelColor = Color.FromArgb(mask.LayerMask.ImageData[pos], mask.LayerMask.ImageData[pos], mask.LayerMask.ImageData[pos]);
        ...
        private static Color XYZToRGB(double X, double Y, double Z)
        {
            ...
            int nRed = (int)Math.Round(var_R * 256.0);
            int nGreen = (int)Math.Round(var_G * 256.0);
            int nBlue = (int)Math.Round(var_B * 256.0);
        ...
        private static Color CMYKToRGB(byte c, byte m, byte y, byte k)
        {
            ...
            int nRed = (int)Math.Round((1.0 - (C * (1 - K) + K)) * 255);
            int nGreen = (int)Math.Round((1.0 - (M * (1 - K) + K)) * 255);
            int nBlue = (int)Math.Round((1.0 - (Y * (1 - K) + K)) * 255);
